...
export default function GreetingHero({ mode, closet, preferences }: GreetingHeroProps) {
  ...
  return (
    <div className="hero-glow relative overflow-hidden rounded-3xl px-5 py-6 text-white shadow-lg sm:px-8 sm:py-8">
      <div className="flex flex-wrap items-center gap-2">
        <span className="rounded-full bg-white/15 px-3 py-1 text-[10px] font-bold uppercase tracking-widest text-white/90">
          {t('packFromOwn')}
        </span>
        <StreakChip />
      </div>
      ...
      <p className="mt-3 max-w-md text-sm text-white/70 sm:text-base">{t('heroSub')}</p>
        ...
        <div className="mt-5 flex gap-3 overflow-x-auto pb-1">
          {preview.items.slice(0, 4).map((item) => (
            <div key={item.id} className="w-20 shrink-0 text-center">
              ...
              <p className="mt-1.5 truncate text-[11px] font-medium text-white/80">{item.name}</p>
            </div>
          ))}
        </div>
      )}
